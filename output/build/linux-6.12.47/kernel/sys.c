...
/* Trace syscall support */

void trace_syscall_entry(int nr)
{
	if (current->syscall_tracing) {
		printk(KERN_INFO "[trace] pid=%d syscall=%d\n",
		       current->pid, nr);
	}
}

SYSCALL_DEFINE1(trace_syscall, int, trace)
{
	current->syscall_tracing = trace;
	return 0;
}

/* end trace syscall support */
